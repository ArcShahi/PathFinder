...
Grid::Grid(int cols, int rows) : m_Cells(cols * rows, CellType::Empty) {
...
void Grid::Draw(Rectangle bounds, const GridColors &colors,
  ...
  Rectangle sr{bounds.x + m_Start.x * cell, bounds.y + m_Start.y * cell, cell,
               cell};
  Rectangle er{bounds.x + m_End.x * cell, bounds.y + m_End.y * cell, cell,
               cell};
...
void Grid::SetWall(int x, int y, bool isWall) {
  if (static_cast<int>(m_Start.x) == x && static_cast<int>(m_Start.y) == y)
    return;
  if (static_cast<int>(m_End.x) == x && static_cast<int>(m_End.y) == y)
    return;
